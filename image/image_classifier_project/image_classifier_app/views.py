...
class ImageUploadView(APIView):
   def post(self,request,*args,**kwargs):
      ...
      if serializer.is_valid():
         ...
         #Extract top predicted class  and its asssociated probability
         top_prediction = decoded_predictions[0][0]
         class_label = top_prediction[1]
         class_probability = top_prediction[2]
         print(class_label)
         return Response({'class_label':class_label,'class_probabilty': class_probability},status=status.HTTP_200_OK)
      else:
